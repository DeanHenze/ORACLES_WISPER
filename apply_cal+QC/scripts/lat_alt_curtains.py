...
# Needed to get the permil symbol for figure labels:
...
"""
def variance_profiles(year): # str, 'yyyymmdd'
    
    ## Get data averaged by 10s blocks and remove data above 6km:
    block_secs = 10
    data = get_data(year, block_secs=block_secs)
    ...
    ## Standard deviation vertical profiles. For each var, get both a 
    ## typical stdev profile and a max stdev profile:
    ##-------------------------------------------------------------------------
    ...
    var_dxs = pd.pivot_table(data, values='dxs_permil', index=['hght_bin'], 
                             columns='lat_bin', aggfunc=np.var)
        # The 'typical' standard deviation profiles are taken as the square-root of 
        # the latitude-averaged variances:
    std_h2o_prf = (var_h2o.mean(axis=1, skipna=True))**0.5
    std_dD_prf = (var_dD.mean(axis=1, skipna=True))**0.5
    std_dxs_prf = (var_dxs.mean(axis=1, skipna=True))**0.5        
        # Maximum standard deviation profiles:
    max_std_prf_dD = (var_dD.max(axis=1, skipna=True))**0.5
    max_std_prf_dxs = (var_dxs.max(axis=1, skipna=True))**0.5
    ##-------------------------------------------------------------------------
    ...
    for col in mean_h2o_prfs.columns.values:
        # dD, d18O precisions are fxns of humidity in units of ppmv. The 
        # division by sqrt(block_secs) is to account for precision improvement 
        # due to the averaging:
        logh2o = np.log(mean_h2o_prfs[col]*1000/0.622)
        precs_dD_prfs[col] = (6*10**6)*(logh2o**-6.69)/(block_secs)**0.5
        prec_18O = 15425*(logh2o**-4.72)/(block_secs)**0.5
    ...
    return {'h2o':(std_h2o_prf,), 'dD':(std_dD_prf, prec_dD_meanprf, max_std_prf_dD), 
            'dxs':(std_dxs_prf, prec_dxs_meanprf, max_std_prf_dxs)} 
...
def curtain_plots(year, curtains, grid_lat, grid_hght):
    ...
    stdax1 = fig.add_axes([0.76,0.74,0.2,0.25]) # H2O std profile
    stdax2 = fig.add_axes([0.76,0.44,0.2,0.25]) # dD std profile
    stdax3 = fig.add_axes([0.76,0.14,0.2,0.25]) # d18O std profile
    ...
    # curtains, (2) for dD and dxs, plot maximum standard deviation profiles.
    # (3) for dxs plot a profile of typical instrument precisions at each height. 
    # Plotted to the right of their respective curtains:
    var_prfs = variance_profiles(year)
    for key, ax in zip(var_prfs.keys(), (stdax1,stdax2,stdax3)):
        ax.plot(var_prfs[key][0], var_prfs[key][0].index, 'k-')
        if key != 'h2o': 
            ax.plot(var_prfs[key][2], var_prfs[key][2].index, 'k--')
        if key == 'dxs':
            ax.plot(var_prfs[key][1], var_prfs[key][1].index, color='grey', 
                    linestyle='solid')
    ...
    plt_sublabels = ['(a)','(b)','(c)']
    if year=='2016': hplacement = -21.5
    if year in ['2017','2018']: hplacement = -11.5
    for ax, l in zip([ax1,ax2,ax3], plt_sublabels):
       ax.text(hplacement, 5.5, l, fontsize=16, 
    ...
    stdax1.set_xlim(0,2.5)
    stdax2.set_xlim(3,300); stdax2.set_xticks([10,100])
    stdax3.set_xlim(0.5,70); stdax3.set_xticks([1,10])
    
    cax_ylabels = ('q [g/kg]', r'$\delta$D '+u'[\u2030]', 'dxs '+u'[\u2030]')
