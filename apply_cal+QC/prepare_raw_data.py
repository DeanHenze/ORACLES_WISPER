...
path_wisper_raw_dir = r"../WISPER_data/raw/"
path_wisper_prep_dir = r"../WISPER_data/basic_prep/"
path_outlier_tintvls_dir = r"../WISPER_data/outlier_time_intervals/"
...
"""
Process a raw data file for 2016. This includes:
    - Take the subset of variables from the raw data to be used in the 
      final ESPO files.
    - Interpolate or group/average to get the data in 1Hz. 
      Grouping/averaging is applicable to the 5Hz instrument, and 
      interpolating is applicable to the 0.5 Hz instrument.   
    - Mask outlying data with flagged value -9999.
    
Input: date (str) 'yyyymmdd'; date of flight
"""
##_____________________________________________________________________________
...
## Interpolate data slower than 1Hz. This is done by first filling in any
## 'missing' rows with a row of np.nan's, then using pandas' interpolation
## function to fill in the nan rows. A max of 3 consecutive nan rows will
## be filled, so rows beyond that are labelled as missing data:
    # First and last time stamps:
    t0 = data0.iloc[0]['Start_UTC']; tf = data0.iloc[-1]['Start_UTC']
    # this reference time var has timestamps @ 1Hz.
    t_ref = pd.DataFrame({'t_ref':np.arange(t0,tf+1,1)})
    # merge data with reference time to get rows of Nan's where needed:
...
## verify that everything went well with averaging and interpolation. Do this
## by verifying that 1) The number of rows in data1 is equal to tf-t0+1, 2)
## the number of non-nan values in 'Start_UTC' is equal to tf-t0+1, and 3)
## the time difference between adjacent rows is 1s for every row. This is
## probably overkill, but no worries it's not computationally expensive. If
## any of these tests fail, replace the data in the 'Start_UTC' column with
## that from the 't_ref' column:
...
## are just some time intervals with missing data. So replace the 
## data in the 'Start_UTC' column with that from the 't_ref' column
## to get a time var with no missing values but keep missing value
## entries for other vars:
        if bool_dt:
...
## Keep only the timestamp, humidity, and isotope ratio columns. Rename them
## to be consistent with the other two ORACLES years:
...
##_____________________________________________________________________________
"""
Process a raw data file for 2017 or 2018. This includes:
...
Note that in the raw files, what I am calling Pic1(Pic2) vars are labelled as 
pic0(pic1) vars. 
...
## Load WISPER data and change missing value flags to np.nan:
    ...
    #data0.loc[:,['f_user_slpm','f_byp_slpm']] =     \
    #    data0[['f_user_slpm','f_byp_slpm']].replace(-99.0,np.nan)
...
## Mask bad data:
    ...
    # Some weird Pic1 behavior (probably mostly in 2017 due to the bad 
    # thermistor attachment). At times dD would become unrealistically high 
    # or humidity would go negative:
